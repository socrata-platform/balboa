...
trait SocrataMetricQueue extends MetricQueue {
  ...
  def logAction(t:MetricQueue.Action.Value, viewUid:ViewUid, domainId:DomainId, value:Int=1, tokenUid:AppToken): Unit = {
  ...
  def logRowAccess(accessType:MetricQueue.AccessChannel.Value, domainId:DomainId, viewUid:ViewUid, count: Int, token:AppToken): Unit = {
