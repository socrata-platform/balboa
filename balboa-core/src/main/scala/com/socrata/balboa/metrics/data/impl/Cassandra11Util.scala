...
import com.netflix.astyanax.serializers.{AnnotatedCompositeSerializer, StringSerializer}
import com.netflix.astyanax.AstyanaxContext.Builder
import com.netflix.astyanax.impl.AstyanaxConfigurationImpl
import com.netflix.astyanax.connectionpool.{LatencyScoreStrategyType, LatencyScoreStrategy, NodeDiscoveryType}
...
object Cassandra11Util {
  ...
  def sliceIterator(queryImpl:Cassandra11Query, entityId:String, period:Period, query:List[Date]):Iterator[Timeslice] = {
  ...
  def metricsIterator(queryImpl:Cassandra11Query, entityId:String, query:scala.collection.immutable.Map[Period, List[Date]]) = {
    {
      for {
        (period, dates: List[Date]) <- query.iterator
        date <- dates.iterator
